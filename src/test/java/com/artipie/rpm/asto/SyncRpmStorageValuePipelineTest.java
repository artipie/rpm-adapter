...
class SyncRpmStorageValuePipelineTest {
    ...
    void processesExistingItem() {
        ...
        new SyncRpmStorageValuePipeline<>(this.asto, key).process(
            (input, out) -> {
                try {
    ...
    void processesExistingLargeSizeItem(
        ...
        new SyncRpmStorageValuePipeline<String>(this.asto, kfrom, kto)
            .processWithResult(
                (input, out) -> {
    ...
    void processesExistingItemAndReturnsResult() {
        ...
        MatcherAssert.assertThat(
            ...
            new SyncRpmStorageValuePipeline<Integer>(this.asto, key).processWithResult(
                (input, out) -> {
                    try {
                        final List<String> list = IOUtils.readLines(input.get(), charset);
                        list.add(2, "seven");
    ...
    void writesNewItemAndReturnsResult() {
        ...
        MatcherAssert.assertThat(
            new SyncRpmStorageValuePipeline<>(this.asto, key).processWithResult(
                (input, out) -> {
                    MatcherAssert.assertThat(
                        "Input should be absent",
                        input.isPresent(),
    ...
    void writesToNewLocation() {
        ...
        new SyncRpmStorageValuePipeline<>(this.asto, read, write).process(
            (input, out) -> {
                try {
                    IOUtils.write(
